...
table_name = os.environ.get('DYNAMODB_TABLE_NAME', 'camera_battery_history')
...
def get_latest_for_all_cameras(cors_headers):
    """
    Get the most recent battery reading for each camera.
    ...
    try:
        ...
        print(f"Error getting latest battery readings: {str(e)}")
        return {
            'statusCode': 500,
            'headers': cors_headers,
            'body': json.dumps({
                'error': 'Failed to retrieve battery readings',
                'message': str(e)
            })
        }
...
def get_camera_history(camera_name, hours, cors_headers):
    """
    Get battery history for a specific camera over the specified number of hours.
    ...
    try:
        ...
        response = table.query(
            KeyConditionExpression=Key('camera_name').eq(camera_name) &
                                 Key('battery_ts').gte(threshold_timestamp),
        ...
        print(f"Error getting camera battery history: {str(e)}")
        return {
            'statusCode': 500,
            'headers': cors_headers,
            'body': json.dumps({
                'error': 'Failed to retrieve camera battery history',
                'message': str(e)
            })
        }
...
def lambda_handler(event, context):
    ...
    API Endpoints:
        GET /battery/latest - Get latest battery reading for all cameras
        GET /battery/history?camera=<name>&hours=<hours> - Get battery history for a camera
    """
    ...
    if path.endswith('/latest'):
        ...
        return {
            ...
            'body': json.dumps({
                'error': 'Not found',
                'message': 'Valid endpoints: /battery/latest, /battery/history'
            })
        }
